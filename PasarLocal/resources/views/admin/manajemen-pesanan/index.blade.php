...
<div class="container py-4">
    ...
    {{-- Navigasi Tab Status --}}
    @php
        $statuses = ['Belum Diproses', 'Diproses', 'Dikirim', 'Selesai', 'Batal'];
    @endphp
    ...
    <div class="tab-content" id="status-tabsContent">
        @foreach ($statuses as $key => $status)
            <div class="tab-pane fade @if($key == 0) show active @endif" id="content-{{ $key }}" role="tabpanel">
                @php $orders = $groupedOrders->get($status, []); @endphp
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('admin.manajemen-pesanan.update', $order->id) }}" method="POST" class="d-flex align-items-center">
                                    @csrf
                                    @method('PUT')
                                    <select name="status_pesanan" class="form-select me-2" required>
                                        @foreach($statuses as $opt)
                                            <option value="{{ $opt }}" {{ $opt == $order->status_pesanan ? 'selected' : '' }}>{{ $opt }}</option>
                                        @endforeach
                                    </select>
